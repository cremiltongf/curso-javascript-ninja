(function() {
    ...
    */
    ...
    function hasClass( markup, cssClass ) {
        let regex = new RegExp( 'class=[\'"](?:[\\w\\s]+)?(' + cssClass + ')(?:[\\w\\s]+)?[\'"]', 'g' );
        let isContain = markup.match( regex ) ? true : false;
        return isContain;
    }
    let classes = [ 'container', 'text', 'date', 'excerpt', 'main' ];
    classes.forEach( function( item ) {
        let check = hasClass( markup, item );
        console.log( `${check} para a classe ${item}` );
    } );
