(function() {
    ...
    */
    ...
    function cleanCPF( cpfs ) {
        // alternativas
        // let regex = /[-. x]/g;
        // let regex = /[\D]/g;
        let regex = /[^\d]/g;
        return cpfs.replace( regex, '');
    }
    ...
    */
    ...
    cpfs.forEach( function( cpf ) {
        console.log( cleanCPF( cpf ).replace( /(\d{3})(\d{3})(\d{3})(\d{2})/gm , '$1.$2.$3-$4' ) );
    });
    ...
    */
    ...
    let listText = text.match( /j\w{3}o/g );
    // alternativa /ju[nl]ho/g
    // let listText = text.match( /ju[nl]ho/g );
    ...
    */
    ...
    let htmlMatch = '<div><section><blockquote>Texto <img /></blockquote></section></div>';
    let listTags = htmlMatch.match( /<\w+>/g );
    ...
    */
    ...
    let htmlNull = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';
    let htmlNullReturn = htmlNull.match( /<\w+><\/\w+>/g );
    console.log( htmlNullReturn );
    ...
    */
    ...
    let contentHtml = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
    let regexHtml = /<(\w+)>([^<]+)<\/(\w+)>/g;
    let replaceHtml = contentHtml.replace( regexHtml, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n');
